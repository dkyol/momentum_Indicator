...
def get_high_volume_data():
    ...
    try:
        ...
        high_volume_tickers = [
            ...
            'NVDA', 'META', 'NFLX', 'AMD', 'QQQ',
            'SPY', 'VOO', 'VTI', 'TQQQ', 'SQQQ',
            'ARKK', 'SOXL', 'SPXL', 'TLT', 'GLD'
        ]
        
        # Get current date and calculate periods
        end_date = datetime.now()
        start_date = end_date - timedelta(days=35)  # Extra days for calculation
        ...
        for ticker in high_volume_tickers:
            try:
                ...
                # Calculate returns
                day_return = ((data['Close'].iloc[-1] / data['Close'].iloc[-2]) - 1) * 100 if len(data) >= 2 else 0
                week_return = ((data['Close'].iloc[-1] / data['Close'].iloc[-6]) - 1) * 100 if len(data) >= 6 else 0  
                month_return = ((data['Close'].iloc[-1] / data['Close'].iloc[-21]) - 1) * 100 if len(data) >= 21 else 0
